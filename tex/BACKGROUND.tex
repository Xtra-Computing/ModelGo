...
Typically, a ML project is constructed with three components: data, software, and models, which are usually governed by different licensing frameworks.
...
Common sources of risk include Wikipedia, arXiv, WordPress, Common Crawl, etc (Refer to Table.~\ref{tab:works} for more examples).
...
However, given that ML, especially Neural Networks (NNs), is still in its emerging stages, the license dependency chain is shorter compared to OSS projects, and most of them use the latest versions of OSS licenses like Apache-2.0 and MIT.
